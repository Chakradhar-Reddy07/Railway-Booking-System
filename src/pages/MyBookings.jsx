...
export default function MyBookings() {
  ...
  const handleCancel = async (ticketId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/bookings/cancel/${ticketId}`, {
  method: "PATCH",
  headers: {
...
// ---------- DEBUG START ----------
const raw = await res.text();   // get raw output
...
// ---------- DEBUG END ------------
  ...
  // Helper convert "A1-23" -> Coach A1, Seat 23
  const renderSeatDetails = (p, t) => {
    ...
    if (t.status === 'PENDING') {
      return <span className="text-orange-500 ml-auto text-sm">Waiting for Payment</span>;
    }
          ...
          list.map((t) => (
            ...
            >
              ...
              <div className="text-sm text-gray-700 dark:text-gray-300 grid grid-cols-2 gap-2">
                <div>
                  <span className="font-semibold">Booking Date:</span> {t.booking_date}
                </div>
                <div>
                  <span className="font-semibold">Travel Date:</span> {t.travel_date}
                </div>
                <div>
                  <span className="font-semibold">From:</span> {t.boarding_station_name} (
                  {t.boarding_station_id})
                </div>
                <div>
                  <span className="font-semibold">To:</span> {t.departure_station_name} (
